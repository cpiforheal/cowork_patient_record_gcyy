...
public final class InventoryPermission {
    ...
    private static final Set<String> MANAGER_ROLES = Set.of("admin", "quality");
    private static final Set<String> STAFF_ROLES = Set.of("admin", "quality", "nurse", "doctor", "frontdesk");

    private InventoryPermission() {}

    public static AuthSessionService.SessionUser requireManager() {
        AuthSessionService.SessionUser user = currentUserOrThrow();
        if (!MANAGER_ROLES.contains(user.role())) {
