...
public class ClinicAiAssistantService {
    ...
    public Map<String, Object> ask(AiAssistantRequest request, AuthSessionService.SessionUser user) {
        ...
        ClinicAiKnowledgeService.KnowledgeSelection knowledge = new ClinicAiKnowledgeService.KnowledgeSelection(List.of(), List.of());
        String model = "";
        ...
        try {
            ...
            ClinicAiConfigService.EffectiveAiConfig aiConfig = aiConfigService.resolveDoubaoConfig();
    ...
    private void logAssistantCall(
        ...
        ClinicAiKnowledgeService.KnowledgeSelection knowledge,
        String model,
        String answer,
        String errorMessage,
        long startedAt,
        ...
        logService.record(
            new ClinicAiAssistantLogService.AssistantLogDraft(
                assistantType,
    ...
    private String buildUserPrompt(
        ...
        payload.put(
            "instruction",
            "请用中文回答。回答系统问题必须优先依据 knowledgeBase；如果知识库没有覆盖某个菜单、权限、流程或制度，必须明确说明“当前系统知识库未覆盖”，不得编造。病历、质控、管理类回答必须区分“系统规则”“当前患者/页面数据”“AI 建议”。只提供说明、清单、草稿和操作建议，不替代诊断、处方、医嘱、正式病历、质控结论或归档审核。"
        );
        try {
    ...
    private ArrayNode compactKnowledge(ClinicAiKnowledgeService.KnowledgeSelection knowledge) {
        ArrayNode result = objectMapper.createArrayNode();
        if (knowledge == null || knowledge.items() == null) return result;
        for (ClinicAiKnowledgeService.KnowledgeItem item : knowledge.items()) {
    ...
    private void requireAllowedRole(String assistantType, AuthSessionService.SessionUser user) {
    ...
    private String systemPrompt(String assistantType) {
        return switch (assistantType) {
            ...
            case "leader" -> """
                ...
                不提供医疗诊断。
                关注谁负责、卡在哪里、建议动作是什么。
                必须优先使用用户消息中的 knowledgeBase 回答系统流程、进销存、备份和权限规则。
                始终用中文回答，简洁可执行。
                """;
            default -> """
                你是院内公共 AI 助手。
                你帮助同事理解系统使用、医院流程、数据组织和日常协作问题。
                你不替代医生判断、处方、诊断、正式病历或最终质控结论。
                涉及医疗风险时，请提示用户找责任医生或质控人员确认。
                必须优先使用用户消息中的 knowledgeBase；知识库未覆盖的系统规则要明确说明未覆盖，不能编造。
                始终用中文回答，温和、清晰、可执行。
                """;
        };
    }
    ...
    public record AiAssistantAttachment(String name, String type, Long size, String dataUrl, String source) {}
}
