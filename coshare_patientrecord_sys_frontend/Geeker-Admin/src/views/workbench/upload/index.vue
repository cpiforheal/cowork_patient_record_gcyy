    ...
    <section class="quick-upload-panel" :class="{ 'is-upload-success': uploadSuccess }">
      ...
      <el-tabs v-else v-model="activeTab">
        <el-tab-pane label="快捷上传" name="upload">
          <div class="quick-form">
            ...
            <FailedUploadList
              ...
              :stale-failed-upload-summaries="staleFailedUploadSummaries"
              :uploading="uploading"
              @restore="restoreFailedUpload"
              @retry="retryFailedUpload"
              @dismiss="dismissFailedUpload"
              @dismiss-stale="dismissStaleFailure"
            />
            <el-result
              v-if="uploadSuccess"
              class="upload-result"
        ...
        <el-tab-pane label="旧共享病历导入" name="legacy">
          <LegacyImportPanel
            v-model:legacy-file-list="legacyFileList"
            v-model:legacy-file-text="legacyFileText"
            v-model:allow-overwrite-legacy-conflicts="allowOverwriteLegacyConflicts"
            :legacy-form="legacyForm"
            :departments="departments"
            :document-types="documentTypes"
            :legacy-candidates="legacyCandidates"
            :classified-legacy-count="classifiedLegacyCount"
            :pending-legacy-count="pendingLegacyCount"
            :previewing-legacy="previewingLegacy"
            :importing="importing"
            :legacy-preview="legacyPreview"
            :patient-match-label="patientMatchLabel"
            :legacy-source-label="legacySourceLabel"
            :import-result="importResult"
            :unassigned-rows="unassignedRows"
            @update:legacy-form="Object.assign(legacyForm, $event)"
            @sync-legacy-files="syncLegacyFiles"
            @preview="previewLegacyImport"
            @commit="commitLegacyImport"
            @clear="clearLegacyForm"
            @view-patient="patientId => router.push(`/patients/detail/${patientId}`)"
            @continue-import="importResult = null"
          />
        </el-tab-pane>
      </el-tabs>
    </section>
...
import { Camera, CirclePlus, Upload, UploadFilled } from "@element-plus/icons-vue";
import {
...
import FailedUploadList from "./components/FailedUploadList.vue";
import LegacyImportPanel from "./components/LegacyImportPanel.vue";
import PatientLookup from "./components/PatientLookup.vue";
import { useUploadWorkbench } from "./composables/useUploadWorkbench";
import { useUploadQueue, type FailedUpload, type UploadItem } from "./composables/useUploadQueue";
...
const {
  ...
  selectedPatient,
  storageKey: UPLOAD_FAILURE_SUMMARY_KEY
});
...
const restoreFailedUpload = (failure: FailedUpload) => {
  ...
  restoreFailedUploadItems(failure);
};
...
const handleInspectionDirectory = async (event: Event) => {
  ...
  inspectionFiles.value = await traceAsync("upload.inspection.prepareFiles", () =>
    mapWithConcurrency(files, async (file, index) => ({
...
const clearInspectionFiles = (resetState = true) => {
  inspectionFiles.value.forEach(item => URL.revokeObjectURL(item.previewUrl));
  inspectionFiles.value = [];
  if (resetState) resetUploadState();
...
const quickUploadDocuments = async (): Promise<UploadDocumentItem[]> => {
  const uploadFiles = uploadItems.value.flatMap(item =>
    item.files.map(file => ({
      file,
      type: item.type,
      typeLabel: documentTypes.find(type => type.value === item.type)?.label || item.type
    }))
  );

  return traceAsync("upload.quick.prepareDocuments", () =>
    mapWithConcurrency(uploadFiles, async ({ file, type, typeLabel }) => ({
      type,
      typeLabel,
      fileName: file.name,
...
const syncLegacyFiles = async (_uploadFile?: UploadFile, uploadFiles?: UploadFile[]) => {
  ...
  legacyCandidates.value = await traceAsync("upload.legacy.prepareFiles", () =>
    mapWithConcurrency(uploadFiles, async (file, index) => {
